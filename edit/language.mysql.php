  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel(3)) {
      ...
      if ($formVars['update'] == -2) {
        $formVars['copyfrom'] = clean($_GET['r_lang_copyfrom'], 10);

        if ($formVars['copyfrom'] > 0) {
          $q_string  = "select r_lang_number,r_lang_specialize ";
          $q_string .= "from r_language ";
          $q_string .= "where r_lang_character = " . $formVars['copyfrom'];
          ...
          while ($a_r_language = mysql_fetch_array($q_r_language)) {
            ...
            $q_string =
              "r_lang_character     =   " . $formVars['r_lang_character']      . "," .
              "r_lang_number        =   " . $a_r_language['r_lang_number']       . "," .
              "r_lang_specialize    =   " . $a_r_language['r_lang_specialize']   . "\"";
      ...
      if ($formVars['update'] == -3) {
        ...
        $output .= "<tr>\n";
        $output .= "  <td class=\"button ui-widget-content\">\n";
        $output .= "<input type=\"button\" name=\"copyitem\"  value=\"Copy Language Table From:\" onClick=\"javascript:attach_language('language.mysql.php', -2);\">\n";
        $output .= "<select name=\"r_lang_copyfrom\">\n";
        ...
        while ($a_runners = mysql_fetch_array($q_runners)) {
          ...
          $q_string .= "from r_language ";
          $q_string .= "where r_lang_character = " . $a_runners['runr_id'] . " ";
          $q_r_language = mysql_query($q_string) or die(header("Location: " . $Siteroot . "/error.php?script=" . $package . "&error=" . $q_string . "&mysql=" . mysql_error()));
          $r_lang_total = mysql_num_rows($q_r_language);

          if ($r_lang_total > 0) {
            $output .= "<option value=\"" . $a_runners['runr_id'] . "\">" . $a_runners['runr_aliases'] . " (" . $r_lang_total . ")</option>\n";
          }
        }
