...
# Description: Retrieve data and update the database with the new info. Prepare and display the table
  ...
  if (isset($_SESSION['username'])) {
    $package = "projacc.mysql.php";
    $formVars['update']        = clean($_GET['update'],     10);
    $formVars['r_proj_id']     = clean($_GET['r_proj_id'],  10);

    if ($formVars['update'] == '') {
      $formVars['update'] = -1;
    }
    if ($formVars['r_proj_id'] == '') {
      $formVars['r_proj_id'] = 0;
    }
      ...
      if ($formVars['update'] == 0) {
        ...
        if ($formVars['r_acc_number'] > 0) {
          ...
          $q_string =
            ...
            "r_acc_parentid    =   " . $formVars['r_proj_id'];

          if ($formVars['update'] == 0) {
            $query = "insert into r_accessory set r_acc_id = NULL," . $q_string;
            $message = "Projectile Accessory added.";
          }
      ...
      if ($formVars['update'] == 2) {
        ...
        if ($formVars['r_ammo_parentid'] > 0) {
          ...
          if ($formVars['update'] == 2) {
            ...
            $message = "Projectile Ammunition associated.";
          }
...
# redisplay the 'myprojectiles' part for the new ammo.
          print "show_file('projectiles.mysql.php?update=-1');\n";
      ...
      $output .= "<tr>\n";
      $output .= "  <th class=\"ui-state-default\">Accessories (" . $formVars['r_proj_id'] . ")</th>\n";
      $output .= "  <th class=\"ui-state-default\" width=\"20\"><a href=\"javascript:;\" onmousedown=\"toggleDiv('projectile-accessories-listing-help');\">Help</a></th>\n";
      $output .= "</tr>\n";
      $output .= "</table>\n";

      $output .= "<div id=\"projectile-accessories-listing-help\" style=\"display: none\">\n";
...
# on new load of data only
      if ($formVars['r_proj_id'] == 0) {
        $output .= "<tr>\n";
        $output .= "  <td class=\"ui-widget-content\" colspan=\"5\">Select one Projectile Weapon in order to purchase accessories.</td>\n";
        $output .= "</tr>\n";
        $output .= "</table>\n";

        print "document.getElementById('projacc_table').innerHTML = '" . mysqli_real_escape_string($db, $output) . "';\n\n";
      } else {

# r_proj_id == the id of the projectile owned/selected. If zero, then no projectile has been selected and no accessories presented.
# then r_proj_number == the id of actual projectile that was selected which gives me the class id.
# show the accessories associated with 'Projectiles' (get from accessory table (acc_type) linked to subjects)
...
# so Projectiles by default
        $where = "where sub_name = \"Projectile\" ";

# get the purchased projectile id
# got the number, now get the proj_class and proj_name from the projectile table
# now get the class name from the class table
        $q_string  = "select proj_class,proj_name,proj_cost,r_proj_character ";
...
# for that item or something that works for all
        $where .= "and (acc_accessory = \"" . $a_r_projectile['proj_name'] . "\" or acc_accessory = \"\") ";
        ...
        if (mysqli_num_rows($q_accessory) > 0) {
          while ($a_accessory = mysqli_fetch_array($q_accessory)) {
            ...
            $linkstart .= "?update=0";
            $linkstart .= "&r_proj_id="         . $formVars['r_proj_id'];
            $linkstart .= "&r_acc_character=" . $a_r_projectile['r_proj_character'];
            $linkstart .= "&r_acc_number="    . $a_accessory['acc_id'];
            $linkstart .= "');";
            $linkstart .= "show_file('projectile.mysql.php";
            $linkstart .= "?update=-1";
            $linkstart .= "&r_proj_character=" . $a_r_projectile['r_proj_character'];
            $linkstart .= "');\">";
            ...
            if ($a_accessory['acc_cost'] == -1) {
              $acc_cost = return_Cost($a_r_projectile['proj_cost']);
            }
...
# now display the available ammo
        ...
        if (mysqli_num_rows($q_r_ammo) > 0) {
          while ($a_r_ammo = mysqli_fetch_array($q_r_ammo)) {
            ...
            $linkstart .= "&r_ammo_parentid=" . $formVars['r_proj_id'];
            $linkstart .= "');";
            $linkstart .= "show_file('projectile.mysql.php";
            $linkstart .= "?update=-1";
            $linkstart .= "&r_proj_character=" . $a_r_projectile['r_proj_character'];
